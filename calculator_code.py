...
def sum():
    if e1.get()=="" or e2.get()=="":
        w=Message(f1,text="This es error").grid(row=0,column=0)
    else:
        output=int(e1.get())+int(e2.get())
        w=Message(f1,text="{}".format(output),height=40,width=80)
...
b_minus=Button(root,text="-",width=16).grid(row=6,column=3)
